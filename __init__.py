...
def main(
    ...
    if train:
        train_model("data/images", "data/masks", epochs=50)
    predict_one(model_path=model_path, image_path=image_path,model_type=model_type)
...
def predict_one(model_path, image_path,model_type="cropper"):
    ...
    if not os.path.exists(model_path):
        # train_model("data/images", "data/masks", epochs=50)
        raise ValueError(f"Model not found at {model_path}. Training completed, but model file is missing.")
...
if __name__ == "__main__":
    ...
    model_path = "pattern_model.pt" if model_type == "cropper" else "segmentation_model.pt"
    if args.directory:
